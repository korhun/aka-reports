...
def resize_if_larger(image, max_dim, interpolation=None):
    ...
    if w > h:
        if w > max_dim:
            return resize(image, width=max_dim, interpolation=interpolation)
        else:
            return image
    else:
        if h > max_dim:
            return resize(image, height=max_dim, interpolation=interpolation)
        else:
            return image
...
def resize_if_smaller(image, min_dim, interpolation=None):
    ...
    if w > h:
        if w < min_dim:
            return resize(image, width=min_dim, interpolation=interpolation)
        else:
            return image
    else:
        if h < min_dim:
            return resize(image, height=min_dim, interpolation=interpolation)
        else:
            return image
...
def fill_polyline_transparent(image, pnts, color, opacity=0.1, thickness=-1):
    ...
    res = cv2.addWeighted(image, 1.0, blk, opacity, 0)
...
def put_text(img, text_, center, color=None, font_scale=0.5, thickness=1, back_color=None, replace_non_ascii=True, thickness_back=None):
...
def change_brightness_contrast(input_img, brightness=0, contrast=0):
    # https://stackoverflow.com/a/50053219/1266873
    if brightness != 0:
        if brightness > 0:
            shadow = brightness
            highlight = 255
...
def _write(mat, file_name):
    ...
    im_buf_arr = None  # noqa - do not delete
...
def read(file_name, flags=cv2.IMREAD_UNCHANGED):
    # cv2.imread
    stream = open(file_name, "rb")
    bytes_ = bytearray(stream.read())
    numpyarray = numpy.asarray(bytes_, dtype=numpy.uint8)
    return cv2.imdecode(numpyarray, flags)


def read_as_color(file_name):
    mat = read(file_name)
    if mat.dtype == 'uint16':
        mat = downcast_image(mat)
...
def read_thumb(file_name):
    ...
    if os.path.isfile(fn_thumb):
        return read_as_color(fn_thumb)
    else:
        return read_as_color(file_name)


def replace_color(image, rgb_source, rgb_target):
    r, g, b = rgb_source[0], rgb_source[1], rgb_source[0]
    red, green, blue = image[:, :, 0], image[:, :, 1], image[:, :, 2]
    mask = (red == r) & (green == g) & (blue == b)
    image[:, :, :3][mask] = rgb_target


def replace_value(image, source, target):
    image[:, :][image == source] = target


def downcast_image(image):
    """
    Downcast 16 bit images to 8 bit by scaling

    Parameters
    ----------
    image (numpy.ndarray of shape (height, width)): 16 bit image array

    Returns
    -------
    image (numpy.ndarray of shape (height, width)): 8 bit image array
    """
